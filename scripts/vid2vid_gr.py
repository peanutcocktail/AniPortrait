...
def main():
    ...
    save_dir_name = f"result"

    save_dir = Path(f"output/{save_dir_name}")
    save_dir.mkdir(exist_ok=True, parents=True)
    ...
    for ref_image_path in config["test_cases"].keys():
        ...
        for source_video_path in config["test_cases"][ref_image_path]:
            ...
            save_path = f"{save_dir}/_noaudio.mp4"
            save_videos_grid(
                video,
                save_path,
                n_rows=3,
            ...
            # ffmpeg.output(stream.video, audio.audio, save_path.replace('_noaudio.mp4', 'result.mp4'), vcodec='copy', acodec='aac').run()
            # Extract audio from the source video and save to 'audio_output', overwrite if exists
            ffmpeg.input(source_video_path).output(audio_output, acodec='copy').overwrite_output().run()

            # Merge audio and video into final output, overwrite if exists
            final_output_path = save_path.replace('_noaudio.mp4', 'result.mp4')
            stream = ffmpeg.input(save_path)
            audio = ffmpeg.input(audio_output)
            ffmpeg.output(stream.video, audio.audio, final_output_path, vcodec='copy', acodec='aac').overwrite_output().run()
